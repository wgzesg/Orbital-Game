...
public class PlayerCharacterController : MonoBehaviour
{
    ...
    void HandleCharacterMovement()
    {
        ...
        bool isSprinting = m_InputHandler.GetSprintInputHeld();
        {
            ...
            Vector3 localspaceMoveInput = m_InputHandler.GetMoveInput();
            Vector3 worldspaceMoveInput = transform.TransformVector(localspaceMoveInput);
            ...
            if (isGrounded)
            {
                ...
                if(characterVelocity.magnitude > 0.5)
                {
                    if (localspaceMoveInput.z < 0f)
                        ...
                        m_Anime.SetInteger("State", 3);  // sprinting
                    else
                        m_Anime.SetInteger("State", 2);  // normal running
                }
                else
    ...
    void UpdateCharacterHeight(bool force)
    {
        ...
        if (force)
        {
            ...
            playerCamera.transform.localPosition = Vector3.up * m_TargetCharacterHeight * cameraHeightRatio + Vector3.back * 3;
